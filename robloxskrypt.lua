...
local function webhook(msg)
  ...
  --GUI
local Window = library:AddWindow("jebac zydow xddd",
  {
	main_color = Color3.fromRGB(80, 80, 80),
	min_size = Vector2.new(373, 433),
	toggle_key = Enum.KeyCode.RightShift,
})
local boothTab = Window:AddTab("Budka")
...
local bThemes = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("ScreenGui"):WaitForChild("BoothSettings"):WaitForChild("ScrollingFrame"):WaitForChild("ChangeTheme"):WaitForChild("Themes")
  ...
  --Booth Settings
local textUpdateToggle = boothTab:AddSwitch("Update tekstu", function(bool)
...
local textUpdateDelay = boothTab:AddSlider("Update tekstu co (S)", function(x)
	if settingsLock then
		return
	end
...
boothTab:AddLabel("Kolor tekstu")
local hexBox = boothTab:AddTextBox("Tylko hex", function(text)
	if settingsLock then
		return
	end
...
boothTab:AddLabel("Zwiekszanie celu co")
local goalBox = boothTab:AddTextBox("Tylko cyferki", function(text)
...
boothTab:AddLabel("Custom tekst")
local customBoothText = boothTab:AddConsole({
	["y"] = 50,
	["source"] = "",
})
boothTab:AddLabel("Gdzie stoisz:")
...
standingPos:Add('Przod')
standingPos:Add('Lewo')
standingPos:Add('Prawo')
standingPos:Add('Tyl')
customBoothText:Set(getgenv().settings.customBoothText)
boothTab:AddButton("Zapisz", function()
...
if signPass then
	local signToggle = signTab:AddSwitch("Wez tabliczke", function(bool)
	...
	local signUpdateToggle = signTab:AddSwitch("Update tekstu", function(bool)
		if settingsLock then
			return
		end
	...
	signTab:AddLabel("Kolor tekstu")
	local signHexBox = signTab:AddTextBox("TYLKO HEX", function(text)
		if settingsLock then
			return
		end
		local success = pcall(function()
	...
	signTab:AddButton("Zapisz", function()
	...
	signTab:AddLabel('NIE STAC CEI NA GAMEPASS ZJEBKU XDDD')
end
  
  --Chat Settings
local autoThanks = chatTab:AddSwitch("Auto dziekowanko", function(bool)
	getgenv().settings.autoThanks = bool
	saveSettings()
end)
autoThanks:Set(getgenv().settings.autoThanks)
local autoBeg = chatTab:AddSwitch("Auto blaganko", function(bool)
	if settingsLock then
		return
	end
...
local thanksDelay = chatTab:AddSlider("Dziekowanko co (S)", function(x)
	if settingsLock then
		return
	end
...
local begDelay = chatTab:AddSlider("Blaganko co (S)", function(x)
	if settingsLock then
		return
	end
...
local tym = chatTab:AddFolder("Dziekowanko")
...
local bm = chatTab:AddFolder("Blaganne wiadomosci:")
...
bm:AddButton("Zapisz", function()
  ...
  --Webhook Settings
local webhookToggle = webhookTab:AddSwitch("Webhook", function(bool)
	getgenv().settings.webhookToggle = bool
	saveSettings()
end)
webhookToggle:Set(getgenv().settings.webhookToggle)

local serverHopDonation = webhookTab:AddSwitch("Powiadomienie po hopce", function(bool)
	getgenv().settings.webhookAfterSH = bool
	saveSettings()
end)

local webhookBox = webhookTab:AddTextBox("Link Webhook", function(text)
...
webhookTab:AddLabel('Wcisnij enter by zapisac')
webhookTab:AddButton("Test", function()
	if getgenv().settings.webhookBox then
		webhook("WEBHOOK DZIALA ZIAAA!!")
	end
end)
...
local serverHopToggle = serverHopTab:AddSwitch("Auto hopka", function(bool)
	if settingsLock then
		return
	end
...
serverHopTab:AddLabel("Czas hopki resetuje sie co mamone")
serverHopToggle:Set(getgenv().settings.serverHopToggle)

local serverHopDelay = serverHopTab:AddSlider("Co ile hopka (M)", function(x)
	if settingsLock then
		return
	end
...
if vcEnabled then
	local vcToggle = serverHopTab:AddSwitch("Serwery z glosem", function(bool)
		if settingsLock then
			return
		end
...
local alhop = serverHopTab:AddSwitch("Losowa hopka", function(bool)
	getgenv().settings.AlternativeHop = bool
	saveSettings()
end)

local sHopSwitch = serverHopTab:AddSwitch('Hopka po mamonie',function(bool)
...
local staffHopSwitch = serverHopTab:AddSwitch('Hopka jak admin?',function(bool)
...
serverHopTab:AddButton("Hopka", function()
	serverHop()
end)

  --Other tab
otherTab:AddLabel('Zatanczysz?')
...
danceDropdown:Add("Nie lubie tanczyc")
danceDropdown:Add("1")
danceDropdown:Add("2")
danceDropdown:Add("3")
local render = otherTab:AddSwitch("Wylacz renderowanie (FW)", function(bool)
...
local jumpswitch = otherTab:AddSwitch("Skakanie za mamone", function(bool)
	getgenv().settings.donationJump = bool
	saveSettings()
end)
jumpswitch:Set(getgenv().settings.donationJump)
local autoReply = otherTab:AddSwitch("Auto odpisywanie", function(bool)
	getgenv().settings.autoNearReply = bool
	saveSettings()
end)
autoReply:Set(getgenv().settings.autoNearReply)
local anMode = otherTab:AddSwitch('Peleryna niewidka', function(bool)
...
local spinToggle = otherTab:AddSwitch('Kreciol',function(bool)
	...
	if getgenv().settings.spinSet then
		...
		Spin.AngularVelocity = Vector3.new(0,0.25 * spinSpeedMultiplier,0)
...
local topOfBooth = otherTab:AddSwitch('Stoj na budce',function(bool)
	getgenv().settings.boothTop = bool
	saveSettings()
end)

topOfBooth:Set(getgenv().settings.boothTop)

local jumpsPerRB = otherTab:AddSlider("Ile skaczesz za 1R$", function(x)
	if settingsLock then
		return
	end
...
local spinMultiplier = otherTab:AddSlider("Kreciol szybkosc", function(x)
	if settingsLock then
		return
	end
...
local function walkToBooth()
	...
	if getgenv().settings.boothTop then
		Players.LocalPlayer.Character.Humanoid.RootPart.CFrame = CFrame.new(mainPosX + Vector3.new(0,10,0))
	end
end
walkToBooth()
...
Players.LocalPlayer.leaderstats.Raised.Changed:Connect(function()
	...
	if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
		...
		webhook(":man_in_manual_wheelchair: Mamona wpadla| Az " .. tostring(Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) .. 'R$ (po podatkach: ' .. tostring(math.floor((Players.LocalPlayer.leaderstats.Raised.Value - RaisedC) * 0.6)) .. 'R$) | Lacznie: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value) .. 'R$ | Konto: ' .. Players.LocalPlayer.DisplayName .. ' (' .. Players.LocalPlayer.Name .. ')')
	end
...
task.spawn(function()
	while task.wait(5) do
		if (Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChildOfClass('Humanoid').RootPart) then
			...
			if (root.Position - positionX.Position).Magnitude > 1000 or (root.Position - positionX.Position).Magnitude < -1000 then
				serverHop()
			end
		end
	end
...
if getgenv().settings.webhookAfterSH then
	webhook('hi hello ' .. Players.DisplayName .. ' (' .. Players.LocalPlayer.Name .. ') serverhopped')
end
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Loading..." then
	game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "jebac zydow xddd"
end
