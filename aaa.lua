...
if isfile and writefile and typeof(isfile) == 'function' and typeof(writefile) == 'function' then
	if not isfile('PromptedDiscordLinoriaCFR.txt') then
		writefile('PromptedDiscordLinoriaCFR.txt', game:GetService('HttpService'):JSONEncode('hi'))
...
boothOtherSettings:AddToggle('boothUpStand', {
	Text = 'Stand on booth top',
	Default = false,
	Tooltip = 'yes',
})
...
local function walkToBooth()
	...
	task.wait(1)
	Players:Chat(Options.DanceChoiceD.Value)
	if Toggles.boothUpStand.Value == true then
